...
def get_comprehensive_metric_descriptions() -> Dict[str, Dict]:
    ...
    return {
        ...
        'utxo_created_value_mean': {
            'name': 'UTXO创建平均价值',
            'desc': '统计新创建的UTXO（未花费交易输出）的平均价值。这个指标反映了每笔交易创建的平均输出金额，能够帮助分析网络中资金流动的粒度。较高的平均值可能表示大额交易增加，较低的平均值则可能表示小额交易活跃'
        },
        'utxo_created_value_median': {
            'name': 'UTXO创建中位数价值',
            'desc': '新创建UTXO的中位数价值。相比平均值，中位数不受极端大额交易的影响，更能反映典型交易的规模。这个指标有助于了解大多数用户的交易行为模式'
        },
        'utxo_created_value_sum': {
            'name': 'UTXO创建总价值',
            'desc': '统计所有新创建UTXO的总价值。这个指标衡量了网络中新产生的可花费输出总量，是评估经济活动强度的重要指标。高值通常表示网络活跃度高'
        },
        'utxo_spent_value_mean': {
            'name': 'UTXO花费平均价值',
            'desc': '统计被花费的UTXO的平均价值。这个指标反映了每笔输入的平均金额，有助于理解资金使用模式。平均花费价值的变化可以反映市场参与者的交易策略变化'
        },
        'utxo_spent_value_median': {
            'name': 'UTXO花费中位数价值',
            'desc': '被花费UTXO的中位数价值。提供了典型交易输入规模的稳健估计，不受大额交易影响。这有助于识别普通用户的消费行为'
        },
        'utxo_spent_value_sum': {
            'name': 'UTXO花费总价值',
            'desc': '所有被花费UTXO的总价值。这个指标反映了网络中资金流动的总规模，是衡量链上经济活动的关键指标。与交易量类似但更精确地反映了实际价值转移'
        },
        'utxo_count': {
            'name': 'UTXO总数',
            'desc': '当前未花费交易输出的总数量。UTXO数量反映了网络中可用资金的分散程度，数量增加可能表示更多的资金分散存储，数量减少可能表示资金整合'
        },
        'utxo_created_count': {
            'name': '新创建UTXO数量',
            'desc': '统计新创建的UTXO数量。这个指标反映了交易输出的创建频率，高值通常表示网络交易活跃，资金分散程度增加'
        },
        'utxo_spent_count': {
            'name': '花费UTXO数量',
            'desc': '统计被花费的UTXO数量。反映了交易输入的使用频率，是衡量网络活动的重要指标。大量UTXO被花费可能表示资金整合或大规模交易活动'
        },
        
        # 区块相关指标
        'block_height': {
            'name': '区块高度',
            'desc': '当前区块链的最新区块高度。这是区块链长度的直接度量，每个新区块都会使高度增加1。区块高度是确定交易确认数和网络同步状态的基础指标'
        },
        'block_count': {
            'name': '区块数量',
            'desc': '在指定时间段内产生的区块数量。这个指标反映了网络的出块频率和稳定性。正常情况下应该接近预期值（如比特币约144块/天）'
        },
        'block_size_mean': {
            'name': '平均区块大小',
            'desc': '区块的平均大小（字节）。反映了网络的使用率和拥堵程度。接近最大限制的平均大小表示网络繁忙，较小的平均大小表示网络相对空闲'
        },
        'block_size_sum': {
            'name': '区块总大小',
            'desc': '所有区块的累计大小。这个指标反映了区块链数据的增长速度和存储需求。对于评估网络扩容需求和节点运行成本很重要'
        },
        'block_interval_mean': {
            'name': '平均出块间隔',
            'desc': '相邻区块之间的平均时间间隔。这个指标反映了网络的出块稳定性。偏离目标值可能表示算力波动或网络问题'
        },
        
        # 盈亏相关指标
        'profit_realized': {
            'name': '已实现盈利',
            'desc': '当币被转移时，如果卖出价格高于买入价格产生的盈利总额。这个指标反映了投资者实际锁定的利润，高值通常出现在牛市获利了结时期'
        },
        'profit_relative': {
            'name': '相对盈利',
            'desc': '已实现盈利占总已实现价值的百分比。这个相对指标能够标准化不同市值规模的比较，更好地反映市场的盈利状况'
        },
        'loss_realized': {
            'name': '已实现亏损',
            'desc': '当币被转移时，如果卖出价格低于买入价格产生的亏损总额。这个指标反映了投资者实际承受的损失，高值通常出现在恐慌性抛售时期'
        },
        'loss_relative': {
            'name': '相对亏损',
            'desc': '已实现亏损占总已实现价值的百分比。这个相对指标有助于评估市场的亏损程度和投降情绪的强度'
        },
        # 地址类指标 - 活跃度
        'active_count': {
            'name': '活跃地址数',
